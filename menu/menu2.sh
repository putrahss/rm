...
Isadmin=$(curl -sS https://raw.githubusercontent.com/ppnhss/hss/main/sc3 | grep $MYIP | awk '{print $5}')
...
Exp1=$(curl https://raw.githubusercontent.com/ppnhss/hss/main/sc3 | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
clear
status=$( curl -sS https://raw.githubusercontent.com/ppnhss/sc3/main/status )
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
serverV=$( curl -sS https://raw.githubusercontent.com/ppnhss/sc3/main/versi )
if [ "$Isadmin" = "ON" ]; then
