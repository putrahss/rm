...
LLatest=`date`
Get_Data () {
git clone https://github.com/ppnhss/userbackup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    cd /root/user-backup
    git config --global user.email "papadaanhss93@gmail.com" &> /dev/null
    git config --global user.name "ppnhss" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m $NameUser &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/ppnhss/userbackup.git
    git push -f https://github.com/ppnhss/userbackup.git &> /dev/null
}
...
echo -e""
echo -e "$yellow Username :👉  ppnhss  👈$NC"
echo -e""
echo -e "$yellow PASTEKAN Username GITHUB DI BAWAH INI$NC"
echo -e ""
Save_And_Exit
fi
link="https://raw.githubusercontent.com/ppnhss/userbackup/main/$NameUser/$NameUser.zip"
sleep 1
...
cekdata=$(curl -sS https://raw.githubusercontent.com/ppnhss/userbackup/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
